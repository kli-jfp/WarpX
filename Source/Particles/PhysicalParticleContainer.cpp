                                                  ...
                                                  ParticleReal z_shift)
{
    ...
    if (ParallelDescriptor::IOProcessor()) {
        ...
        int num_chunks = 1; // Default value

        try {
            // Attempt to get the 'num_chunks' attribute
            openPMD::Attribute num_chunks_attr = series->getAttribute("num_chunks");
            num_chunks = num_chunks_attr.get<int>();
        } catch (const openPMD::no_such_attribute_error&) {
            // Attribute does not exist, use default value
            num_chunks = 1;
        } catch (const std::bad_cast&) {
            // Attribute exists but is not of type int, use default value
            num_chunks = 1;
        }

        int chunk_size = npart / num_chunks;
        ...
        for (int chunk = 0; chunk < num_chunks; ++chunk) {
            ...
            std::size_t end = static_cast<std::size_t>((chunk == num_chunks - 1) ? npart : start + chunk_size);
